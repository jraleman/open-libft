...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/02/19 04:12:46 by jaleman           #+#    #+#             */
/*   Updated: 2017/02/19 04:12:47 by jaleman          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			print_hexadecimal(va_list ap, t_prntf *attr, int fd)
{
	...
	if (attr->flags & LOW_P_BIT)
		len = format_unsigned((uintmax_t)va_arg(ap, void *), attr, fd, \
			&conv_hex);
	else if (attr->flags & J)
		len = format_unsigned(va_arg(ap, uintmax_t), attr, fd, &conv_hex);
	else if (attr->flags & Z)
		len = format_unsigned(va_arg(ap, size_t), attr, fd, &conv_hex);
	else if (attr->flags & LL)
		len = format_unsigned(va_arg(ap, unsigned long long), attr, fd, \
			&conv_hex);
	else if (attr->flags & L)
		len = format_unsigned(va_arg(ap, unsigned long), attr, fd, &conv_hex);
		...
		len = format_unsigned(va_arg(ap, unsigned int), attr, fd, &conv_hex);
	return (len);
}
